/*******************************************************************************
 * Name            : i_jpeg_handler.cc
 * Project         : image_tools
 * Module          : io_handler
 * Description     : Implementation of JPEG I/O handler
 ...
 * Static Functions
 ******************************************************************************/
PixelBuffer* IJPEGHandler::LoadImage(const std::string file_name) {
  ...
  loaded_image_buffer = new PixelBuffer(cinfo.output_width, cinfo.output_height,
...
bool IJPEGHandler::SaveImage(const std::string file_name,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ColorData curr_pixel = buffer->get_pixel(x, y);
      image_buffer[((height-(y+1))*width+x)*3] =
          static_cast<JSAMPLE>((curr_pixel.red()*255.0));
      image_buffer[((height-(y+1))*width+x)*3+1] =
          static_cast<JSAMPLE>((curr_pixel.green()*255.0));
      image_buffer[((height-(y+1))*width+x)*3+2] =
          static_cast<JSAMPLE>((curr_pixel.blue()*255.0));
    }
  }
